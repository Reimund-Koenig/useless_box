...
struct ModeStartSequence_under_test : public box::ModeStartSequence {
    ModeStartSequence_under_test(box::Servomanager* box_servomanager_mock,
                          box::ModeFunctionJitter* box_mode_function_jitter_mock,
                          box::Wait* box_wait_mock,
                          box::Switch* box_switch_mock) :
    ModeStartSequence(box_servomanager_mock,box_mode_function_jitter_mock, box_wait_mock, box_switch_mock) {}
};

class TestModeStartSequence : public ::testing::Test {
  protected:
    ModeStartSequence_under_test* mode_start_sequence_under_test;
    ...
    virtual void SetUp() {
        ...
        mode_start_sequence_under_test = new ModeStartSequence_under_test(
    ...
    virtual void TearDown() {
        delete mode_start_sequence_under_test;
    ...
    virtual void runstart_sequenceSequence() {
        // 1 - Reset both motors
        ...
        EXPECT_FALSE(mode_start_sequence_under_test->run());
        ...
        EXPECT_FALSE(mode_start_sequence_under_test->run()); // run jitter speed 2-> false
        EXPECT_FALSE(mode_start_sequence_under_test->run()); // run jitter speed 2-> false
        EXPECT_FALSE(mode_start_sequence_under_test->run()); // run jitter  speed 2-> true

        EXPECT_CALL(*box_mode_function_jitter_mock, run(_,_,_,_,_)).WillRepeatedly(Return(true));
        EXPECT_FALSE(mode_start_sequence_under_test->run()); // run jitter speed 3 -> true
        EXPECT_FALSE(mode_start_sequence_under_test->run()); // run jitter speed 4 -> true
        EXPECT_FALSE(mode_start_sequence_under_test->run()); // run jitter speed 5 -> true
        EXPECT_FALSE(mode_start_sequence_under_test->run()); // run jitter speed 6 -> true
        ...
        EXPECT_CALL(*box_servomanager_mock, move_pilot_servo_to_percent(0,6));
        EXPECT_CALL(*box_servomanager_mock, move_copilot_servo_to_percent(0,6));
        EXPECT_TRUE(mode_start_sequence_under_test->run());
    }
};

TEST_F(TestModeStartSequence, test_wait_init) { EXPECT_TRUE(true); }

TEST_F(TestModeStartSequence, test_OLD_start_with_low_switchstate) {
    // move switch to start state -> if low nothing happens
    EXPECT_CALL(*box_switch_mock, is_high()).WillOnce(Return(false));
    EXPECT_FALSE(mode_start_sequence_under_test->run());

    // now run the start_sequence sequence
    runstart_sequenceSequence();
}

TEST_F(TestModeStartSequence, test_OLD_start_with_high_switchstate) {
    // move switch to start state -> if low nothing happens
