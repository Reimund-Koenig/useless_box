...
TEST_F(TestServomanager, test_servomanager_move_pilot_servo_percentage) {
    ...
    EXPECT_CALL(*box_lower_servo_mock, set_speed_and_get_sleeptime(2)).WillOnce(Return(100));
    EXPECT_CALL(*box_wait_mock, milliseconds(100));
    ...
    EXPECT_CALL(*box_upper_servo_mock, set_speed_and_get_sleeptime(4)).WillOnce(Return(200));
    EXPECT_CALL(*box_wait_mock, milliseconds(200));
...
TEST_F(TestServomanager, test_servomanager_move_copilot_servo_percentage) {
    ...
    EXPECT_CALL(*box_switch_mock, is_high()).WillOnce(Return(false));
    EXPECT_CALL(*box_upper_servo_mock, move_to_percent(expected_result)).Times(1);
    EXPECT_CALL(*box_upper_servo_mock, set_speed_and_get_sleeptime(5)).WillOnce(Return(300));
    EXPECT_CALL(*box_wait_mock, milliseconds(300));
    EXPECT_CALL(*box_lower_servo_mock, move_to_percent(_)).Times(0);
    EXPECT_CALL(*box_lower_servo_mock, set_speed_and_get_sleeptime(_)).Times(0);
    ...
    EXPECT_CALL(*box_switch_mock, is_high()).WillOnce(Return(true));
    EXPECT_CALL(*box_lower_servo_mock, move_to_percent(expected_result)).Times(1);
    EXPECT_CALL(*box_lower_servo_mock, set_speed_and_get_sleeptime(6)).WillOnce(Return(1000));
    EXPECT_CALL(*box_wait_mock, milliseconds(1000));
    EXPECT_CALL(*box_upper_servo_mock, move_to_percent(_)).Times(0);
    EXPECT_CALL(*box_upper_servo_mock, set_speed_and_get_sleeptime(_)).Times(0);
...
TEST_F(TestServomanager, test_servomanager_move_both_servo_percentage) {
    ...
    EXPECT_CALL(*box_lower_servo_mock, set_speed_and_get_sleeptime(4)).WillOnce(Return(1400));
    EXPECT_CALL(*box_wait_mock, milliseconds(1400));
    EXPECT_CALL(*box_upper_servo_mock, move_to_percent(expected_result_copilot)).Times(1);
    EXPECT_CALL(*box_upper_servo_mock, set_speed_and_get_sleeptime(5)).WillOnce(Return(2000));
    EXPECT_CALL(*box_wait_mock, milliseconds(2000));
    ...
    EXPECT_CALL(*box_upper_servo_mock, set_speed_and_get_sleeptime(3)).WillOnce(Return(1));
    EXPECT_CALL(*box_wait_mock, milliseconds(1));
    EXPECT_CALL(*box_lower_servo_mock, move_to_percent(expected_result_copilot)).Times(1);
    EXPECT_CALL(*box_lower_servo_mock, set_speed_and_get_sleeptime(2)).WillOnce(Return(2));
    EXPECT_CALL(*box_wait_mock, milliseconds(2));
    servomanager_under_test->move_pilot_servo_to_percent(42, 3);
    servomanager_under_test->move_copilot_servo_to_percent(50, 2);
    // // otherway arround
    EXPECT_CALL(*box_switch_mock, is_high()).WillRepeatedly(Return(false));
    EXPECT_CALL(*box_lower_servo_mock, move_to_percent(expected_result_pilot)).Times(1);
    EXPECT_CALL(*box_lower_servo_mock, set_speed_and_get_sleeptime(1)).WillOnce(Return(42));
    EXPECT_CALL(*box_wait_mock, milliseconds(42));
    EXPECT_CALL(*box_upper_servo_mock, move_to_percent(expected_result_copilot)).Times(1);
    EXPECT_CALL(*box_upper_servo_mock, set_speed_and_get_sleeptime(2)).WillOnce(Return(2000));
    EXPECT_CALL(*box_wait_mock, milliseconds(2000));
    ...
    EXPECT_CALL(*box_switch_mock, is_high()).WillRepeatedly(Return(false));
    EXPECT_CALL(*box_lower_servo_mock, move_to_percent(expected_result_pilot)).Times(1);
    EXPECT_CALL(*box_lower_servo_mock, set_speed_and_get_sleeptime(5)).WillOnce(Return(555));
    EXPECT_CALL(*box_wait_mock, milliseconds(555));
    EXPECT_CALL(*box_upper_servo_mock, move_to_percent(expected_result_copilot)).Times(1);
    EXPECT_CALL(*box_upper_servo_mock, set_speed_and_get_sleeptime(6)).WillOnce(Return(2000));
    EXPECT_CALL(*box_wait_mock, milliseconds(2000));
...
TEST_F(TestServomanager, test_servomanager_box_servos_not_reached_switch) {
    ...
    EXPECT_CALL(*box_upper_servo_mock, current_angle_smaller_than_95_percent()).WillOnce(Return(false));
    EXPECT_FALSE(servomanager_under_test->box_servos_not_reached_switch());

    EXPECT_CALL(*box_upper_servo_mock, current_angle_smaller_than_95_percent()).WillOnce(Return(true));
    EXPECT_CALL(*box_lower_servo_mock, current_angle_smaller_than_95_percent()).WillOnce(Return(false));
    EXPECT_FALSE(servomanager_under_test->box_servos_not_reached_switch());
}
