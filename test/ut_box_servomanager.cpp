...
TEST_F(TestServomanager, test_servomanager_box_servos_not_reached_switch) {
    EXPECT_CALL(*box_upper_servo_mock, current_angle_smaller_than_x_percent(_)).WillOnce(Return(true));
    EXPECT_CALL(*box_lower_servo_mock, current_angle_smaller_than_x_percent(_)).WillOnce(Return(true));
    EXPECT_TRUE(servomanager_under_test->box_servos_not_reached_switch());

    EXPECT_CALL(*box_upper_servo_mock, current_angle_smaller_than_x_percent(_)).WillOnce(Return(false));
...
TEST_F(TestServomanager, test_servomanager_move_motors_blocking_timer) {
    ...
    EXPECT_CALL(*box_upper_servo_mock, move()).Times(6);
    EXPECT_CALL(*box_lower_servo_mock, move()).Times(6);
